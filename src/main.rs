...
fn main() {
    ...
    let amount_of_threads = arguments.threads;
    let (tx, rx) = channel();
    for i in 0..amount_of_threads {
        let tx = tx.clone();
        thread::spawn(move || {
            scan(tx, i, arguments.ip_addr, amount_of_threads);
        });
    }
    let mut out = vec![];
    drop(tx);
...
fn scan(tx: Sender<u16>, start_port: u16, addr: IpAddr, amoount_of_threads: u16) {
    ...
    loop {
        ...
        if MAX - port < amoount_of_threads {
            break;
        }
        port += amoount_of_threads;
    }
}
